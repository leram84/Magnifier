...
public class MainActivity extends Activity implements TextureView.SurfaceTextureListener {
    ...
    private ScrollView translationPanel;
    private TextView translationText;
    private boolean translateActive = false;
    private LiveTranslateController liveTranslate;
    private Handler frameHandler;          // posts periodic frame capture
    private static final long FRAME_INTERVAL_MS = 500; // capture every ~500 ms

    // Language list shown in the picker (display name → ML Kit tag)
    private static final String[] LANG_NAMES = {
            "Afrikaans", "Arabic", "Bengali", "Chinese (Simplified)", "Dutch",
    ...
    private static final String[] LANG_TAGS = {
            TranslateLanguage.AFRIKAANS, TranslateLanguage.ARABIC, TranslateLanguage.BENGALI,
            TranslateLanguage.CHINESE, TranslateLanguage.DUTCH, TranslateLanguage.ENGLISH,
            TranslateLanguage.FINNISH, TranslateLanguage.FRENCH, TranslateLanguage.GERMAN,
            TranslateLanguage.GREEK, TranslateLanguage.HEBREW, TranslateLanguage.HINDI,
            TranslateLanguage.HUNGARIAN, TranslateLanguage.INDONESIAN, TranslateLanguage.ITALIAN,
            TranslateLanguage.JAPANESE, TranslateLanguage.KOREAN, TranslateLanguage.MALAY,
            TranslateLanguage.NORWEGIAN, TranslateLanguage.POLISH, TranslateLanguage.PORTUGUESE,
            TranslateLanguage.ROMANIAN, TranslateLanguage.RUSSIAN, TranslateLanguage.SPANISH,
            TranslateLanguage.SWAHILI, TranslateLanguage.SWEDISH, TranslateLanguage.THAI,
            TranslateLanguage.TURKISH, TranslateLanguage.UKRAINIAN, TranslateLanguage.VIETNAMESE
    };

    private int selectedLangIndex = 5; // default: English
    // ─────────────────────────────────────────────────────────────────────────
    ...
    @Override public void onCreate(Bundle state) {
        ...
        liveTranslate = new LiveTranslateController(translateCallback);
        frameHandler = new Handler(getMainLooper());
    ...
    private void buildInterface() {
        ...
        FrameLayout.LayoutParams torchLayout = new FrameLayout.LayoutParams(dp(64), dp(64),
                Gravity.BOTTOM | Gravity.RIGHT);
        torchLayout.setMargins(0, 0, dp(24), dp(24));
        root.addView(torchButton, torchLayout);

        // ── Translate toggle button ──────────────────────────────────────────
        translateButton = label("⟺ Translate", 16);
        translateButton.setGravity(Gravity.CENTER);
        translateButton.setContentDescription("Toggle live translation");
        translateButton.setOnClickListener(v -> toggleTranslate());
        FrameLayout.LayoutParams transLayout = new FrameLayout.LayoutParams(-2, dp(48),
                Gravity.BOTTOM | Gravity.LEFT);
        transLayout.setMargins(dp(16), 0, 0, dp(24));
        root.addView(translateButton, transLayout);

        // ── Language picker button ───────────────────────────────────────────
        langButton = label(LANG_NAMES[selectedLangIndex], 14);
        langButton.setGravity(Gravity.CENTER);
        langButton.setContentDescription("Select target language");
        langButton.setOnClickListener(v -> showLanguagePicker());
        FrameLayout.LayoutParams langLayout = new FrameLayout.LayoutParams(-2, dp(40),
                Gravity.BOTTOM | Gravity.LEFT);
        langLayout.setMargins(dp(16), 0, 0, dp(76));
        root.addView(langButton, langLayout);

        // ── Translation result panel ─────────────────────────────────────────
        translationText = new TextView(this);
        translationText.setTextSize(17);
        translationText.setTextColor(Color.WHITE);
        translationText.setPadding(dp(14), dp(10), dp(14), dp(10));
        translationText.setText(getString(R.string.no_text_detected));

        translationPanel = new ScrollView(this);
        translationPanel.setBackgroundColor(0xCC000000);
        translationPanel.addView(translationText,
                new FrameLayout.LayoutParams(-1, -2));
        translationPanel.setVisibility(View.GONE);

        FrameLayout.LayoutParams panelLayout = new FrameLayout.LayoutParams(-1, dp(140),
                Gravity.BOTTOM);
        panelLayout.bottomMargin = dp(120); // above the bottom buttons
        root.addView(translationPanel, panelLayout);

        setContentView(root);
    }

    // ── Live translate helpers ────────────────────────────────────────────────

    private void toggleTranslate() {
        translateActive = !translateActive;
        if (translateActive) {
            liveTranslate.setTargetLanguage(LANG_TAGS[selectedLangIndex]);
            liveTranslate.start();
            translationPanel.setVisibility(View.VISIBLE);
            translateButton.setText("⟺ Translate ON");
            translateButton.setTextColor(0xff69f0ae); // green accent
    ...
    private void scheduleFrameCapture() {
        ...
        frameHandler.postDelayed(() -> {
            if (!translateActive || preview == null) return;
            Bitmap frame = preview.getBitmap();
            if (frame != null) {
                liveTranslate.processFrame(frame);
                frame.recycle(); // LiveTranslateController copies internally
            }
            scheduleFrameCapture(); // reschedule
        }, FRAME_INTERVAL_MS);
    }
    ...
    private void showLanguagePicker() {
        new AlertDialog.Builder(this)
                ...
                .setSingleChoiceItems(LANG_NAMES, selectedLangIndex, (dialog, which) -> {
                    selectedLangIndex = which;
                    langButton.setText(LANG_NAMES[selectedLangIndex]);
                    if (translateActive) {
    ...
    private final LiveTranslateController.ResultCallback translateCallback =
            new LiveTranslateController.ResultCallback() {
        @Override public void onTranslation(String text) {
            translationText.setText(text);
        }
        @Override public void onNoText() {
            translationText.setText(getString(R.string.no_text_detected));
        }
        @Override public void onStatus(String statusMsg) {
            if (statusMsg != null) {
                translationText.setText(statusMsg);
            }
        }
    };
    ...
    @Override protected void onPause() {
        // Stop translate mode so resources are released when backgrounded
        if (translateActive) stopTranslate();
        closeCamera();
