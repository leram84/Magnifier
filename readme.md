...
The canonical repair branch consolidates the app name, launcher icon, camera
orientation fix, isolated swipe controls, and no-cache APK workflow on top of
...
2. Select **Build Android APK (no cache)** in the left sidebar.
...
5. Under **Artifacts**, select the newest **magnifeye-v1.4-build-N** artifact.
...
6. Extract the ZIP and open the similarly named
   `Magnifeye-v1.4-build-N.apk` on the Android device.
...
The workflow avoids the Gradle caching action entirely, downloads a fresh Gradle
distribution, deletes all previous build output, and forces every task to run
again. The app displays `v1.4` at the top of its camera
screen. If it does not, the
